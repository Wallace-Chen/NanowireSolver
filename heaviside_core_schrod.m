...
function inside = heaviside_core_schrod(core_side_length, x, y)
    ...
    delta = avg_side_length_schrod / 5;
    
    %% parameter check
    if abs(degree_of_polygon) ~= 3 && degree_of_polygon ~= 6 && degree_of_polygon ~= 0
       disp("Error: heaviside_core_schrod, the geometry not supported!");
       return;
    end
...
%                  (y >= -sqrt(3)*x-core_side_length*sqrt(3)/3 - 2*delta);
    ...
    if debug
       ...
       % disp(fprintf("debug: heaviside_core_schrod, (%f, %f) is reported to be within the region with length %f, degree %d: %d\n", x, y, core_side_length, degree_of_polygon, inside)); 
    end
